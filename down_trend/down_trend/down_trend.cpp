...
{
	int nRet = 0;
	double dVal = ve_bo(nRet, d_max_percent, gia_hien_tai);
	while (std::round(dVal) < std::round(gia_mua))
	{
		nRet++;
		dVal = ve_bo(nRet, d_max_percent, gia_hien_tai);
	}
	std::cout << "so tien le " << dVal;
	return nRet;
}
...
int main()
{
	int nSophienvebo = so_phien_de_ve_bo(36, 20, 9.9);
